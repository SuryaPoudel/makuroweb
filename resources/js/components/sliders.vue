...
<div id='loadingover' v-show="this.popuploader"> </div>
              ...
              <router-link class="btn btn-xs btn-light" v-bind:to="{path: '/slider'}">
    ...
    <input type="text" class="form-control" v-model="searchkey" v-on:keyup="SearchSliders()"  placeholder="Search Photos">
<div v-show="this.loaderactive" class="lds-facebook"><div></div><div></div><div></div></div>
        ...
        <td>
          <router-link class="btn btn-light btn-sm" v-bind:to="{path: 'slider/'+p.id}"><i class="fas fa-pen-square"></i> </router-link>
...
//delete is done here
let uri=window.hostname+'/api/slider/'+this.deleteid;
...
searchResult: function()
{
  ...
  let uri=window.hostname+'/api/searchslider?q='+ query;
...
{   let uri=window.hostname+'/api/slider';
    Axios.get(uri).then((response) => {
      this.posts = response.data;
