...
<div class="table-responsive">
 ...
 <i v-on:click="changeinput()" class="btnUndo fa fa-undo"></i>
                      ...
                      <b-form-input v-if="this.album=='add'" v-model="createalbum" id="slider"
                                  type="text"
      ...
      <b-form-select  v-else  v-model="album">
        <option value="null">Slider</option>
...
deleteRow(index) {
  ...
  updateImage()
    {   
      ...
      let newphoto;
         let uri = window.hostname+'/api/slider/'+this.$route.params.id;
...
if(this.createalbum!=null)
{
  ...
  this.$router.push({path: '/sliders'});
     ...
     getAlbum: function(){
        let uri=window.hostname+'/api/slidertype';
          axios.get(uri)
          .then(response => {
 ...
 selectphotobyid: function($id)
    ...
    let uri = window.hostname+'/api/slider/'+this.$route.params.id;
        ...
        // alert(response);
          ...
          {
        this.editphoto=window.hostname+"/slider/"+response.data.image;
        }
